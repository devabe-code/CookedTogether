extends CharacterBody3D

@export var SENSITIVITY = 0.004
@export var BASE_FOV = 75.0

var speed
const WALK_SPEED = 5.0
const SPRINT_SPEED = 8.0
const MIN_JUMP_VELOCITY = 3.2
const MAX_JUMP_VELOCITY = 9.8

#bob variables
const BOB_FREQ = 2.4
const BOB_AMP = 0.08
var t_bob = 0.0

#fov variables
const FOV_CHANGE = 1.5

# Get the gravity from the project settings to be synced with RigidBody nodes.
var gravity = 20.8

var is_jumping = false
var jump_timer = 0.0
const MAX_JUMP_HOLD_TIME = 0.3
const JUMP_HOLD_FORCE = 30.0

@onready var head = $Head
@onready var camera = $Head/FirstPersonCamera
@onready var debug_camera = $DebugViewPort/SubviewPort/DebugCam

func _ready():
	Input.set_mouse_mode(Input.MOUSE_MODE_CAPTURED)

func _unhandled_input(event):
	if event is InputEventMouseMotion:
		head.rotate_y(-event.relative.x * SENSITIVITY)
		camera.rotate_x(-event.relative.y * SENSITIVITY)
		camera.rotation.x = clamp(camera.rotation.x, deg_to_rad(-40), deg_to_rad(60))

# Bunny hop tuning variables
const AIR_ACCEL = 0.6
const AIR_CONTROL = 0.5
const GROUND_ACCEL = 14.0
const BUNNY_HOP_GRAVITY = 24.0

func _physics_process(delta):
	# Apply gravity and manage jumping
	if not is_on_floor():
		velocity.y -= BUNNY_HOP_GRAVITY * delta
	else:
		is_jumping = false
		jump_timer = 0.0

	# Start Jump
	if Input.is_action_just_pressed("jump") and is_on_floor():
		velocity.y = MIN_JUMP_VELOCITY
		is_jumping = true
		jump_timer = 0.0

	# Hold Jump
	if is_jumping and Input.is_action_pressed("jump") and jump_timer < MAX_JUMP_HOLD_TIME:
		velocity.y += JUMP_HOLD_FORCE * delta
		jump_timer += delta

	# Sprint
	if Input.is_action_pressed("sprint"):
		speed = SPRINT_SPEED
	else:
		speed = WALK_SPEED
	
	# DEBUG CAM
	debug_camera.global_transform.origin = global_transform.origin + Vector3(0, 5, -10)
	debug_camera.look_at(global_transform.origin + Vector3(0, 2, 0), Vector3.UP)
	var cam_target = global_transform.origin + Vector3(0, 5, -10)
	debug_camera.global_transform.origin = debug_camera.global_transform.origin.lerp(cam_target, delta * 5.0)
	debug_camera.look_at(global_transform.origin + Vector3(0, 2, 0), Vector3.UP)



	# Movement
	var input_dir = Input.get_vector("left", "right", "up", "down")
	var direction = (head.transform.basis * transform.basis * Vector3(input_dir.x, 0, input_dir.y)).normalized()
	var accel = GROUND_ACCEL if is_on_floor() else AIR_ACCEL
	var control = 1.0 if is_on_floor() else AIR_CONTROL
	if direction.length() > 0.01:
		velocity.x = lerp(velocity.x, direction.x * speed, delta * accel * control)
		velocity.z = lerp(velocity.z, direction.z * speed, delta * accel * control)
	else:
		velocity.x = lerp(velocity.x, 0.0, delta * accel)
		velocity.z = lerp(velocity.z, 0.0, delta * accel)

	# Head bob
	t_bob += delta * velocity.length() * float(is_on_floor())
	camera.transform.origin = _headbob(t_bob)

	# FOV
	var velocity_clamped = clamp(velocity.length(), 0.5, SPRINT_SPEED * 2)
	var target_fov = BASE_FOV + FOV_CHANGE * velocity_clamped
	camera.fov = lerp(camera.fov, target_fov, delta * 8.0)

	move_and_slide()

func _headbob(time) -> Vector3:
	var pos = Vector3.ZERO
	pos.y = sin(time * BOB_FREQ) * BOB_AMP
	pos.x = cos(time * BOB_FREQ / 2) * BOB_AMP
	return pos
